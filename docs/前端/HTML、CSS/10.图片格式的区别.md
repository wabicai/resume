...
特点：**无损压缩、质量高、体积大、支持透明。**包括（PNG-8和PNG-24），24的效果最好。
...
介绍：APNG（Animated Portable Network Graphics）诞生于2004年，是PNG的位图动画扩展。可以简单地理解为PNG格式的动画版。（本来是用来代替gif格式的，但是因为很多浏览器不支持，无法推广。）
...
特点：年轻的全能型选手，**旨在加快图片加载速度**的图片格式，它**支持有损压缩和无损压缩****（结合了JPG、PNG、GIF的优点，但是老版本的浏览器兼容不好。）**
...
一：根据透明划分：JPG不支持透明。GIF、PNG、Webp支持。

二：大图片且色彩比较丰富可选用JPG。小图 / 色彩简单的可采用png。icon、图标等采用svg。

...
首先，我们先对图片格式按照图片类型和压缩方式进行分类：

#### **图片类型：**

1. 位图：位图又叫像素图，计算机屏幕上的图是由屏幕上的发光点（即像素）构成的，每个点用二进制数据来描述其颜色与亮度等信息，他是离散的。同时因为多个像素的色彩组合就形成了图片。常见位图有 **JPG、PNG、GIF** 等格式。
2. 矢量图：矢量图又叫向量图，它是由一系列计算机指令来描述和记录一幅图，生成的矢量图**文件存储量很小，特别适用于文字设计、图案设计**等，而在前端中比较常用的矢量图有 **SVG** 等格式。

#### **压缩方式：**

1. 无损压缩：是对文件本身的压缩，使得图片的储存空间变小，不会损坏图片。**PNG**就是无损压缩。
2. 有损压缩：是对图片本身的改变，会损坏图片。**JPG**就是有损压缩。

接下来，我们再根据各种图片的区别进行详细的说明。
...
**不适用于所含颜色少、颜色对比强烈、简单的图片。**

**颜色丰富、有渐变，选择 JPG（大小区别很大，JPG 比较小）**

缺点：

1. 当它处理**矢量图形**和 **Logo** 等线条感较强、颜色对比强烈的图像时，人为压缩导致的图片模糊会相当明显。
2. 不支持透明度处理。

#### **PNG/aPng：**

特点：**无损压缩、质量高、体积大、支持透明。**包括（PNG-8 和 PNG-24），24 的效果最好。

> PNG 就是为了取代 GIF 而生的（静态）

应用场景：复杂的、色彩层次丰富的图片，用 PNG 来处理的话，成本会比较高，我们一般会交给 JPG 去存储。我们主要用它来呈现**小的 Logo、颜色简单且对比强烈的图片或背景**等。
...
**特点：**显示效果好；压缩比大；对于颜色信息多的图片（如照片）不能大幅压缩；兼容性不好（特别不好）、支持动画

介绍：APNG（Animated Portable Network Graphics）诞生于 2004 年，是 PNG 的位图动画扩展。可以简单地理解为 PNG 格式的动画版。（本来是用来代替 gif 格式的，但是因为很多浏览器不支持，无法推广。）

#### **SVG：（iconfont 就是一种 SVG）**

特点：**文本文件、体积小、不失真、兼容性好**

优点：

1. SVG 与 PNG 和 JPG 相比，**文件体积更小，可压缩性更强**。
2. 图片可无限放大而不失真
3. **SVG 是文本文件**。我们既可以像写代码一样定义 SVG，把它写在 HTML 里、成为 DOM 的一部分，也可以把对图形的描述写入以 .svg 为后缀的独立文件（SVG 文件在使用上与普通图片文件无异）

局限性:

1. 渲染成本比较高
2. SVG 存在着其它图片格式所没有的学习成本（它是可编程的）
3. SVG 文件通常是极小的，但是当图形的复杂度变高的时候，SVG 文件大小会随之上升，因为 SVG 在渲染的时候需要比像素图更多的计算能力，这也意味着**性能的损耗。所以在 Logo 等图上，应尽可能简洁。**

#### **Base64：**

特点：**文本文件、依赖编码、小图标解决方案**Base64 是作为雪碧图的补充而存在的。**Base64 并非一种图片格式，而是一种编码方式，它类似于雪碧图，是作为小图标解决方案而存在的。**

- 通过对图片进行 Base64 编码，我们可以直接将编码结果写入 HTML 或者写入 CSS，从而减少 HTTP 请求的次数。（浏览器是可以理解这个字符串的，它自动就将这个字符串解码为了一个图片，而不需再去发送 HTTP 请求）

应用场景：**非常小的 Logo**。Base64 编码后，图片大小会膨胀为原文件的 4/3
...
特点：年轻的全能型选手，**旨在加快图片加载速度**的图片格式，它**支持有损压缩和无损压缩\*\***（结合了 JPG、PNG、GIF 的优点，但是老版本的浏览器兼容不好。）\*\*

优点：

1. 支持透明（有损和无损都可），支持动态图片
...
**特点：不仅支持静止图片，也支持动态图片。支持透明背景。体积小，但是色域小，只支持 256 种颜色。**
...
一：根据透明划分：JPG 不支持透明。GIF、PNG、Webp 支持。

二：大图片且色彩比较丰富可选用 JPG。小图 / 色彩简单的可采用 png。icon、图标等采用 svg。
...
![img](https://user-gold-cdn.xitu.io/2018/9/14/165d7a6a7f6a4285?imageView2/0/w/1280/h/960/format/webp/ignore-error/1)

而具体到**图片**这一类的资源，平均请求体积是这样的：

![img](https://user-gold-cdn.xitu.io/2018/9/14/165d7a930d3eb13f?imageView2/0/w/1280/h/960/format/webp/ignore-error/1)
...
![img](https://user-gold-cdn.xitu.io/2018/9/14/165d8aaddff6b9bb?imageView2/0/w/1280/h/960/format/webp/ignore-error/1)

京东首页也不例外：

![img](https://user-gold-cdn.xitu.io/2018/9/14/165d8b82ad50c6a4?imageView2/0/w/1280/h/960/format/webp/ignore-error/1)
...
![img](https://user-gold-cdn.xitu.io/2018/9/15/165db5f3b8a80839?imageView2/0/w/1280/h/960/format/webp/ignore-error/1)

较小的 Logo：

![img](https://user-gold-cdn.xitu.io/2018/9/15/165db6625dbfaae3?imageView2/0/w/1280/h/960/format/webp/ignore-error/1)

颜色简单、对比度较强的透明小图也在 PNG 格式下有着良好的表现：

![img](https://user-gold-cdn.xitu.io/2018/9/15/165db6c6bcf059ed?imageView2/0/w/1280/h/960/format/webp/ignore-error/1)
...
![img](https://user-gold-cdn.xitu.io/2018/9/15/165dbe673eba8fba?imageView2/0/w/1280/h/960/format/webp/ignore-error/1)
...
```html
<img
  src="https://user-gold-cdn.xitu.io/2018/9/15/165db7e94699824b?w=22&h=22&f=png&s=3680"
/>
```
